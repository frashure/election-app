...
var endorsementsController = {
    ...
    postEndorsement: (req, res) => {
        let date = new Date();
        var results = db.query(`INSERT INTO endorsements (voter_id, candidate_id, election_id, dateEndorsed) VALUES (?, ?, ?, ?)`,[req.user.voter_id, req.body.candidate_id, req.body.election_id, date], (err, results) => {
            if (err) {
                console.log(err);
                console.log(req.user);
                res.send(err);
    ...
    deleteEndorsement: (req, res) => {
        var results = db.query(`DELETE FROM endorsements WHERE voter_id = ? AND candidate_id = ? AND election_id = ?`, [req.user.voter_id, req.body.candidate_id, req.body.election_id], (err, results) => {
            if (err) {
                console.log(err);
                res.send(err);
            }
